...
static int onGetUpdatesJSON(esp_http_client_handle_t client, void *context, char *buf, uint16_t len)
{
    ...
    {
        if (!cJSON_IsTrue(cJSON_GetObjectItem(root, "ok")))
            goto fail;
        ...
        if (date != NULL)
        {
            ...
            //ESP_LOGI(TAG, "xxx %s, %d, 0x%llX, %lld", date, tm.tm_year, respDate >> 32, respDate & 0xFFFFFFFF);
            //I (15448) app: xxx Thu, 04 Nov 2021 15:52:34 GMT, 121, 0x49FB, 1636041154

            respDate = respDate & 0xFFFFFFFF;
        }

        char *url = new char[256];

        if (respDate - _msgDate->valueint > 2)
        {
            ESP_LOGI(TAG, "too late {update_id=%d, msgId=%d, fromId=%d, msgText=%s, msgDate=%d, respDate=%lld}",
                     _update_id->valueint, _msgId->valueint, _fromId->valueint, _msgText->valuestring,
                     _msgDate->valueint,
                     respDate);

            char text[64];
            sprintf(text, "too+late+%llds", respDate - _msgDate->valueint);

            sprintf(url, API_SEND_MESSAGE, _fromId->valueint, text, _msgId->valueint);
        }
        else
        {
            ESP_LOGD(TAG, "{update_id=%d, msgId=%d, fromId=%d, msgText=%s, msgDate=%d, respDate=%lld}",
                     _update_id->valueint, _msgId->valueint, _fromId->valueint, _msgText->valuestring,
                     _msgDate->valueint,
                     respDate);
